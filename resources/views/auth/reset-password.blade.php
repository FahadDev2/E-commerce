...
@endif
	...
	<form  class="register-form outer-top-xs"  method="POST" action="{{ route('password.update') }}">
		...
		<div class="form-group">
		    ...
		    <input   class="form-control unicase-form-control text-input"  id="email" type="email" name="email" value="{{$request->email}}" :value="old('email', $request->email)" required
			 autofocus  >
		</div>

		<div class="form-group">
		    <label class="info-title" for="exampleInputEmail1">New Password <span>*</span></label>
		    <input id="password" type="password" name="password" class="form-control unicase-form-control text-input"  required autocomplete="new-password"  >
		</div>
